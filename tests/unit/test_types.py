...
from mwpc_exact import ExactCommitResult, ExactCommitStatus, ExactnessScope, Proposal


def finite_scope() -> ExactnessScope:
    return ExactnessScope(kind="finite_support", description="top-8 per masked slot", top_k=8)
...
def test_scope_records_top_k() -> None:
    scope = finite_scope()
    assert scope.kind == "finite_support"
...
def test_optimal_requires_certificate() -> None:
    with pytest.raises(ValueError):
        ExactCommitResult(
            status=ExactCommitStatus.OPTIMAL,
...
def test_valid_optimal_result() -> None:
    ...
    assert result.status is ExactCommitStatus.OPTIMAL
...
def test_timeout_cannot_claim_selected_proposals() -> None:
    with pytest.raises(ValueError):
        ExactCommitResult(
            status=ExactCommitStatus.TIMEOUT,
            exactness_scope=finite_scope(),
            selected_proposal_ids=("p0",),
        )
