#!/usr/bin/env python3
"""Run the configured M3 randomized correctness campaign."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=REPOSITORY_ROOT / "configs/exact_commit/m3_differential.toml",
    )
    ...
    configured_output = REPOSITORY_ROOT / config["output"]["summary"]
    configured_failures = REPOSITORY_ROOT / config["output"]["failure_directory"]
    output = configured_output if arguments.output is None else arguments.output
    failure_directory = (
        configured_failures if arguments.failure_dir is None else arguments.failure_dir
    )
    ...
    summary = run_differential_campaign(
        ...
        metadata={
            ...
            "support_policy": "generated_explicit_per_position",
